import Link from "next/link";
import { ArrowRight, Quote } from "lucide-react";
import { SectionHeader, Card, StarRating, OptimizedImage } from "@/components/ui";
import { fetchRatingInfo, fetchGoogleReviews } from "@/lib/google-places";

export default async function TestimonialsPreview() {
  const [ratingInfo, reviews] = await Promise.all([
    fetchRatingInfo(),
    fetchGoogleReviews(),
  ]);

  const displayReviews = reviews.slice(0, 3);

  return (
    <section className="section-padding bg-secondary-100">
      <div className="container-custom">
        {/* Header with Rating */}
        <div className="mb-12 text-center">
          <SectionHeader
            title="Apa Kata Mereka"
            subtitle="Pengalaman nyata dari pelanggan yang telah berkunjung ke Sinar Heritage"
          />

          {/* Google Rating */}
          <div className="mt-6 inline-flex items-center gap-3 rounded-full bg-white px-6 py-3 shadow-warm">
            <svg
              className="h-6 w-6"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                fill="#4285F4"
              />
              <path
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                fill="#34A853"
              />
              <path
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                fill="#FBBC05"
              />
              <path
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                fill="#EA4335"
              />
            </svg>
            <StarRating rating={ratingInfo.average} showValue />
            <span className="text-sm text-neutral-600">
              ({ratingInfo.total} ulasan)
            </span>
          </div>
        </div>

        {/* Testimonials Grid */}
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {displayReviews.map((review, index) => (
            <Card key={index} className="relative">
              {/* Quote Icon */}
              <Quote className="absolute right-4 top-4 h-8 w-8 text-brand-100" />

              {/* Rating */}
              <StarRating rating={review.rating} className="mb-4" />

              {/* Review Text */}
              <p className="mb-6 line-clamp-4 text-neutral-600">
                &ldquo;{review.text}&rdquo;
              </p>

              {/* Author */}
              <div className="flex items-center gap-3">
                <div className="relative h-12 w-12 overflow-hidden rounded-full bg-secondary-100">
                  {review.authorPhoto ? (
                    <OptimizedImage
                      src={review.authorPhoto}
                      alt={review.authorName}
                      fill
                      containerClassName="h-full"
                      className="h-full w-full"
                    />
                  ) : (
                    <div className="flex h-full w-full items-center justify-center bg-brand-100 text-lg font-bold text-brand-700">
                      {review.authorName.charAt(0)}
                    </div>
                  )}
                </div>
                <div>
                  <div className="font-semibold text-brand-800">
                    {review.authorName}
                  </div>
                  <div className="text-sm text-neutral-500">
                    {review.relativeTime}
                  </div>
                </div>
              </div>
            </Card>
          ))}
        </div>

        {/* CTA */}
        <div className="mt-12 text-center">
          <Link
            href="/testimonials"
            className="inline-flex items-center gap-2 font-medium text-brand-700 transition-colors hover:text-brand-800"
          >
            Lihat Semua Testimoni
            <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </div>
    </section>
  );
}
